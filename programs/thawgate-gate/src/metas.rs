...
//! `allowlist entry`. [`Layout::of`] is the single source for both writing the list and reading it back.
...
impl Layout {
    pub fn of(check_blacklist: bool, allowlist_mode: AllowlistMode) -> Self {
        let allowlist_on = allowlist_mode != AllowlistMode::Off;
        let mut next = POLICY_INDEX + 1;
        let mut slot = |on: bool| {
            on.then(|| {
                next += 1;
                next - 1
            })
        };
        let issuer = slot(check_blacklist || allowlist_on);
        let blacklist = slot(check_blacklist);
        let allowlist = slot(allowlist_on);
        Layout { issuer, blacklist, allowlist, len: next }
    }

    pub fn for_policy(policy: &GatePolicy) -> Self {
        Self::of(policy.check_blacklist, policy.allowlist_mode)
    }
}
...
mod tests {
    ...
    fn policy(check_blacklist: bool, allowlist_mode: AllowlistMode) -> GatePolicy {
        GatePolicy {
    ...
    fn layout_places_only_enabled_accounts() {
        use AllowlistMode::*;
        let l = |b, a| Layout::of(b, a);
        assert_eq!(l(false, Off), Layout { issuer: None, blacklist: None, allowlist: None, len: 7 });
        assert_eq!(l(true, Off), Layout { issuer: Some(7), blacklist: Some(8), allowlist: None, len: 9 });
        assert_eq!(l(false, AllowOnly), Layout { issuer: Some(7), blacklist: None, allowlist: Some(8), len: 9 });
        assert_eq!(l(true, AllowOnly), Layout { issuer: Some(7), blacklist: Some(8), allowlist: Some(9), len: 10 });
        assert_eq!(l(true, BypassForPdas), l(true, AllowOnly));
        for (b, a) in [(false, Off), (true, Off), (false, AllowOnly), (true, AllowOnly)] {
            let p = policy(b, a);
            assert_eq!(POLICY_INDEX + extra_metas(&p).unwrap().len(), Layout::for_policy(&p).len);
        }
    }

    /// Resolves the written metas the way Token ACL does (spl-tlv `ExtraAccountMeta::resolve`) and checks they
    /// land on this program's policy PDA and on sss-token's real registry PDAs (its own seeds and program ID).
    #[test]
    fn metas_resolve_to_sss_token_registry_pdas() {
        let p = policy(true, AllowlistMode::AllowOnly);
        let (mint, owner) = (p.mint, Pubkey::new_unique());
        let mut keys = vec![Pubkey::new_unique(), Pubkey::new_unique(), mint, owner, Pubkey::new_unique(), Pubkey::new_unique()];
